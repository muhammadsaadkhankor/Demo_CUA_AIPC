...
#     """Execute action. Returns False when agent should stop."""
...
# def _check_state(app_name: str) -> str:
#     """Screenshot + narrow classification question. No planning, no actions —
#     just OPEN_FOREGROUND / OPEN_BACKGROUND / CLOSED. This is deliberately the
#     ONLY thing asked of the model in this call, since a 2B VLM answers a
#     single yes/no-style question far more reliably than it plans."""
#     url, _ = capture_screen()
#     _log(f"📸 checking whether '{app_name}' is open...")
...
# def _bring_to_foreground(app_name: str, max_tries: int = 2) -> bool:
...
#         pyautogui.click(x, y)
#         time.sleep(1.0)
#         if _check_state(app_name) == "OPEN_FOREGROUND":
...
# def _search_and_open(app_name: str, max_confirm_tries: int = 5,
#                       confirm_wait: float = 1.5) -> bool:
...
# def _run_generic_flow(instruction: str, step_budget: int | None = None) -> bool:
...
#         try:
#             keep_going = execute_action(action, img_size)
#         except Exception as exc:
...
#         opened = _bring_to_foreground(app_name)
#     else:
...
# # Common name → the words that actually show up in that app's window title.
# # Extend this as you hit apps whose window title doesn't contain the name
# # you speak/type (e.g. "explorer" → "File Explorer").
...
#     candidates = _APP_ALIASES.get(a, [a])
#     return any(c in t for c in candidates) or a in t
...
# # tiny taskbar icons. The VLM is reserved for what it's actually needed
# # for: finding click coordinates and driving follow-up actions. If the
# # window API is unavailable, or a window title doesn't match any known
# # alias, this falls back to the original vision-based check.
...
#     except Exception:
#         active = None
#     if active is not None and _title_matches(app_name, active.title or ""):
...
#         windows = []
#     for w in windows:
#         if w.title and _title_matches(app_name, w.title):
